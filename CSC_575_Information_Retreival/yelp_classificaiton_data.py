...
os.chdir('C:/Users/syarlag1/Desktop/yelp_dataset_challenge_academic_dataset')
...
binary5nntime = end_d5nnbinary - start_d5nnbinary
...
binary9nntime = end_d9nnbinary - start_d9nnbinary
...
binary9nntime = end_31nnbinary - start_31nnbinary
...
#******************************************************************************
def dataframe(path):
    '''Take the path of the json file and then creates a dataframe with 
    the reviews and ratings from the file
    '''
...
def preprocessing(df):
    '''Takes the previously created dataframe and preprocesses the data:
    stemming, stop word removal, punctuation removal.
    '''
...
#******************************************************************************
def dictionaries(df):
    '''This function takes the preprocessed dataframe and creates inverted index'
    as dictionaries: a rating dictionary with the ID and rating values; the tf dictionary
    with words, postings, and their corresponding term frequencies; a tf_idf dictionary
    with words, postings, and their corresponding tfxidf values; inverted_index dictionary with
    the words and their corresponding postings; norm_tf with documents with their normalization 
    factors using term frequencies; and  norm_tf_idf with documents and their normalization 
    factors using tfxidf values
    '''
...
def roc(w = tf_idf, sim = 'cosine'):
    '''with the term weights specified, this function finds the required centroid vectors,
    calculates similarity with the test documents using Rocchio's method, predicts the 
    document category, and then returns the prediction accuracy
    '''
...
def knn(k = 5, w=tf_idf, sim = 'cosine'):
    '''Takes a value for k (5 is default) and calculates the predicted rating of the 
    test documents based on its cosine similarity with the training documents. In the 
    where there is a tie, we pick the higher rating as the predicted rating
    '''
    global allSim
    global accKnn
    global ratingCounts
